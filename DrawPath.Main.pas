...
uses
  ...
  HGM.Button, Vcl.StdCtrls, Vcl.Menus;
...
var W:Integer;
    ...
    Line:TLine;
begin
 if not FLines.GetLine(FLineUnderMouseUID, Line) then Exit;
  ...
  begin
   ...
   for i:= 0 to FLines.Count-1 do
    begin
     ...
     RT:=Rect(AbsolutePoint(FLines[i].PointA).X,
              AbsolutePoint(FLines[i].PointA).Y,
              AbsolutePoint(FLines[i].PointB).X,
              AbsolutePoint(FLines[i].PointB).Y);
     RT.NormalizeRect;
     RT.Inflate(3, 3);
  ...
  imLine:
    begin
     if Button = mbLeft then
      begin
       if FDrawCurrent then
        begin
         //Если неприрывно, то не устанавливать False
         //FDrawCurrent:=False;
         if FCurentLineStart = FFieldPos then Exit;
     ...
     if Button = mbRight then
      begin
       if FDrawCurrent then FDrawCurrent:=False
       else
        begin
...
begin
 ...
 if FLines.GetLine(FLineUnderMouseUID, Line) then
  LabelLineUnderMouse.Caption:=Format('Линия под курсором: A(%d:%d), B(%d:%d)', [DisplayFieldPos(Line.PointA).X, DisplayFieldPos(Line.PointA).Y, DisplayFieldPos(Line.PointB).X, DisplayFieldPos(Line.PointB).Y])
 else
